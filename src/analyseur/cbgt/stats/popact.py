...
class PopAct(object):
    ...
    def count_allspikes_per_bin(cls, spiketimes_set, binsz):
        ...
        with the indicator function :math:`1(.)`, bin width :math:`\\Delta t`, number of bins :math:`\\lceil(T_\\text{max} - T_\\text{min})/\\Delta t\\rceil` such that :math:`T_\\text{min} = \\text{min}(S_\\text{all})`, :math:`T_\\text{max} = \\text{max}(S_\\text{all})`, :math:`S_\\text{all} = \\bigcup_{i=1}^{n_\\text{nuc}} S_i, S_i = \\{t_{i1, t_{i2}, \\ldots, t_{iK_i}\\}\\right}`, and bin edges :math:`b_k = T_\\text{min} + k\\Delta t` for :math:`k=0,1,\\ldots,K`. The bin centers are :math:`c_k = (b_k + b_{k+1})/2`.
